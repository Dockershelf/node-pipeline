...
#
# Usage:
#   ./scripts/debian-smoke-test.sh --dist trixie --node 22 --dist-dir ../node-pipeline/dist
...
if [[ -z "$DIST" || -z "$NODE" || -z "$DIST_DIR" ]]; then
    echo "usage: $0 --dist trixie --node 22 --dist-dir path/to/debs" >&2
    exit 1
fi
...
echo "smoke test passed for node${NODE} on ${DIST}"
